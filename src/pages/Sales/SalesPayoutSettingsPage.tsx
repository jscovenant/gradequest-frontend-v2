  ...
  return <><TopNav sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} title="Payout Settings" />
    <div className="container-fluid"><div className="row"><Sidebar sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
      <main className="col-md-9 col-lg-10 ms-auto db-main sales-work-main">{loading && <Loader message="Loading payout settings..." />}<div className="sales-work-shell">
        <section className="sales-work-hero"><div><div className="sales-work-eyebrow"><i className="bi bi-bank" /> Payout account</div><h1>My Payout Settings</h1><p>Add and verify the bank account where GradeQuest will send your approved sales commissions.</p></div><div className="sales-work-actions"><button className="sales-work-btn sales-work-btn-light" onClick={load}><i className="bi bi-arrow-repeat" /> Refresh</button></div></section>
        ...
        <section className="sales-work-panel"><div className="sales-work-head"><div><h2>Payout History</h2><p>Track transfer batches created by Super Admin.</p></div></div><div className="sales-work-table-wrap"><table className="sales-work-table"><thead><tr><th>Reference</th><th>Status</th><th>Amount</th><th>Items</th><th>Initiated</th><th>Paid</th></tr></thead><tbody>{payouts.map((item) => <tr key={item.id}><td><strong>{item.reference}</strong><br /><small>{item.failure_reason || ""}</small></td><td><span className={`sales-work-pill ${item.status === "paid" ? "sales-work-green" : item.status === "failed" ? "sales-work-red" : "sales-work-gold"}`}>{item.status}</span></td><td>{currency.format(Number(item.total_amount || 0))}</td><td>{item.commission_count}</td><td>{fmtDate(item.initiated_at)}</td><td>{fmtDate(item.paid_at)}</td></tr>)}</tbody></table>{!loading && payouts.length === 0 && <div className="sales-work-empty">No payout has been created yet.</div>}</div></section>
        <Footer /></div></main>
    </div></div></>;
}
