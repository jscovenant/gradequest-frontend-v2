...
export default function StaffQrAttendancePage() {
  ...
  const requestCameraPermission = async () => {
    try {
      setCameraError(null);
      if (isInsecureHttp) {
        setCameraError("Modern browsers require HTTPS (SSL) for live camera access. Please use HTTPS or use manual code entry below.");
        return;
      }
      ...
      if (name === "NotAllowedError" || name === "PermissionDeniedError") {
        setCameraError("Camera permission denied. Tap the lock/tune icon beside your address bar to allow camera access.");
          ...
          <main className="col-md-9 col-lg-10 ms-auto db-main">
            ...
            <div className="db-grid2">
              ...
              <div className="db-panel">
                ...
                <div className="db-card-body">
                  {isInsecureHttp && (
                    <div className="alert alert-warning mb-3" style={{ fontSize: 13, borderRadius: 12 }}>
                      <i className="bi bi-shield-exclamation me-2" />
                      <strong>Notice:</strong> You are viewing this page over plain <code>http://</code>. Modern mobile browsers (Chrome/Safari) require <strong>HTTPS (SSL)</strong> or domain certificate for live camera access. If camera does not start, please use the <strong>Manual Entry</strong> below.
                    </div>
                  )}

                  <div className="scanWrap">
                    {cameraError ? (
                      <div className="scanPaused" style={{ padding: "24px 16px", textAlign: "center" }}>
                        <i className="bi bi-camera-video-off text-danger" style={{ fontSize: 38 }} />
                        <div style={{ fontWeight: 800, fontSize: 15, marginTop: 8, color: "#991b1b" }}>
                          Camera Access Notice
                        </div>
                        <div style={{ fontSize: 12.5, maxWidth: 320, margin: "8px auto 14px", color: "#64748b", lineHeight: 1.5 }}>
                          {cameraError}
                        </div>
                        <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
                          ...
                          >
                            <i className="bi bi-camera-video" /> Try Enabling Camera
                          </button>
                        </div>
                        <div style={{ fontSize: 11.5, color: "#94a3b8", marginTop: 14, lineHeight: 1.5, textAlign: "left", background: "#f8fafc", padding: "10px 14px", borderRadius: 8 }}>
                          <strong>How to allow camera on your device:</strong><br />
                          • <b>Android / Chrome:</b> Tap the tune/lock icon in the URL bar → Permissions → Camera → Allow.<br />
                          • <b>iPhone / Safari:</b> Tap <b>AA</b> or page settings in URL bar → Website Settings → Camera → Allow.
                        </div>
                      </div>
                    ) : cameraOn ? (
                      <Scanner
                        ...
                        onError={(err) => {
                          ...
                          setCameraError(
                            isInsecureHttp
                              ? "Live camera requires HTTPS (SSL). Modern browsers block camera on plain HTTP IP addresses. Please use manual entry or connect via domain HTTPS."
                              : "Camera error or permission denied. Please allow camera permissions in your browser."
                          );
                          setCameraOn(false);
                        }}
                        styles={{
                      ...
                      <div className="scanPaused" style={{ padding: 24, textAlign: "center" }}>
                        <i className="bi bi-camera-video-off" style={{ fontSize: 38 }} />
                        <div style={{ fontWeight: 800, fontSize: 15, marginTop: 8 }}>Scanner Ready</div>
                        <div style={{ fontSize: 12.5, opacity: 0.85, margin: "6px 0 16px" }}>
                        ...
                        <button
                          ...
                          onClick={() => void requestCameraPermission()}
                        >
                          <i className="bi bi-camera-video" /> Start Camera Scanner
                        </button>
                      </div>
                    )}
